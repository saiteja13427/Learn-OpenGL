...
int main (){
    ...
    std::cout << "Attribs: " << nAttribs << std::endl;
    ...
    /* EBO tells OpenGL about the indices of the vertices to draw*/
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        /* Set uniform value which will be used by fragment shaders */
        float timeValue = glfwGetTime();
        float greenVal = (sin(timeValue) / 2.0f) + 0.5f;
        int colorUniformLoc = glGetUniformLocation(shaderProgram, "color");
        glUseProgram(shaderProgram);
        glUniform4f(colorUniformLoc, 0.0f, greenVal, 0.0f, 1.0f);
